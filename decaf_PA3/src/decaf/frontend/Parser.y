...
Variable        :	Type IDENTIFIER
					{
						//VarDef vdef, String ident, TypeLiteral type, Location loc
...
Type            :	INT
					{
						//TypeLiteral type, TypeIdent(int typeTag, Location loc)
...
SimpleStmt      :	LValue '=' Expr
                ...
                |	PLUS_PLUS Expr	%prec Prefix
                	{
                		$$.stmt = new Tree.Preinc($2.expr, $1.loc);
                	}
                |	MINUS_MINUS Expr	%prec Prefix
                	{
                		$$.stmt = new Tree.Predec($2.expr, $1.loc);
                	}
                |	Expr PLUS_PLUS	%prec Suffix
                	{
                		$$.stmt = new Tree.Postinc($1.expr, $1.loc);
                	}
                |	Expr MINUS_MINUS	%prec Suffix
                	{
                		$$.stmt = new Tree.Postdec($1.expr, $1.loc);
              		}
                |	/* empty */
                	{
                		$$ = new SemValue();
...
Expr            :	LValue	 %prec	UMINUS
					{
						$$.expr = $1.lvalue;
					}
				|	PLUS_PLUS Expr	%prec Prefix
                	{
                		$$.expr = new Tree.Preinc($2.expr, $1.loc);
                	}
                |	MINUS_MINUS Expr	%prec Prefix
                	{
                		$$.expr = new Tree.Predec($2.expr, $1.loc);
                	}
                |	Expr PLUS_PLUS	%prec Suffix
                	{
                		$$.expr = new Tree.Postinc($1.expr, $1.loc);
                	}	
                |	Expr MINUS_MINUS	%prec Suffix	
                	{
                		$$.expr = new Tree.Postdec($1.expr, $1.loc);
              		}
                |	Expr '?' Expr ':' Expr
                	{
                		$$.expr = new Tree.CondExpr($1.expr, $3.expr, $5.expr, $2.loc);
                	}
                |	Call
                |	Constant
                |	Expr '+' Expr
...
SwitchStmt		: 	SWITCH '(' Expr ')' '{' CaseList DefaultClause '}'
					{
...
DefaultClause	:	DEFAULT ':' StmtList
					{
						$$.stmt = new Tree.DefaultClause($3.slist, $1.loc);
					}
				|	/* empty */				%prec EMPTY
					{
					 	$$ = new SemValue();
