...
public class Tac {
	...
	public static final Comparator<Tac> COMPARATOR = new Comparator<Tac>() {
		public int compare(Tac o1, Tac o2) {
			if (o1.bbNum < o2.bbNum)
				return -1;
			if (o1.bbNum > o2.bbNum)
				return 1;
			if (o1.id < o2.id)
	...
	private String binanyOpToString(String op) {
		return id + ":    " + op0.name + " = (" + op1.name + " " + op + " " + op2.name + ")";
	}

	private String unaryOpToString(String op) {
		return id + ":    " + op0.name + " = " + op + " " + op1.name;
	}
	...
	public String toString() {
		...
		case LES:
			return binanyOpToString("<");
		case ASSIGN:
			return id + ":    " + op0.name + " = " + op1.name;
		case LOAD_VTBL:
			return id + ":    " + op0.name + " = VTBL <" + vt.name + ">";
		case INDIRECT_CALL:
			if (op0 != null) {
				return id + ":    " + op0.name + " = " + " call " + op1.name;
			} else {
				return id + ":    " + "call " + op1.name;
			}
		case DIRECT_CALL:
			if (op0 != null) {
				return id + ":    " + op0.name + " = " + " call " + label.name;
			} else {
				return id + ":    " + "call " + label.name;
			}
		case RETURN:
			if (op0 != null) {
				return id + ":    " + "return " + op0.name;
			} else {
				return id + ":    " + "return <empty>";
			}
		case BRANCH:
			return id + ":    " + "branch " + label.name;
		case BEQZ:
			return id + ":    " + "if (" + op0.name + " == 0) branch " + label.name;
		case BNEZ:
			return id + ":    " + "if (" + op0.name + " != 0) branch " + label.name;
		case LOAD:
			if (op2.value >= 0) {
				return id + ":    " + op0.name + " = *(" + op1.name + " + " + op2.value + ")";
			} else {
				return id + ":    " + op0.name + " = *(" + op1.name + " - " + (-op2.value)
						+ ")";
			}
		case STORE:
			if (op2.value >= 0) {
				return id + ":    " + "*(" + op1.name + " + " + op2.value + ") = " + op0.name;
			} else {
				return id + ":    " + "*(" + op1.name + " - " + (-op2.value) + ") = "
						+ op0.name;
			}
		case LOAD_IMM4:
			return id + ":    " + op0.name + " = " + op1.value;
		case LOAD_STR_CONST:
			return id + ":    " + op0.name + " = " + MiscUtils.quote(str);
		case MEMO:
			return id + ":    " + "memo '" + str + "'";
		case MARK:
			return id + ":    " + label.name + ":";
		case PARM:
			return id + ":    " + "parm " + op0.name;
		default:
