		...
		do {
			...
			for (BasicBlock bb: bbs) {
				for (int i = 0; i < 2; i++) {
					...
					Iterator<Entry<Tac, Temp>> iterator = bbs.get(bb.next[i]).liveInExt.entrySet().iterator();   
					while (iterator.hasNext()){   
						Entry<Tac, Temp> entry = iterator.next();
						if (!bb.liveOutExt.containsKey(entry.getKey()))
							bb.liveOutExt.put(entry.getKey(), entry.getValue());
					}
				}
				bb.liveOut.removeAll(bb.def);
				
				Iterator<Entry<Tac, Temp>> iter = bb.liveOutExt.entrySet().iterator();
				while (iter.hasNext()) {
					Entry<Tac, Temp> entry = iter.next();
					if(bb.def.contains(entry.getValue())){
						iter.remove();
					}
				}
				...
				Iterator<Entry<Tac, Temp>> iterator_ = bb.liveOutExt.entrySet().iterator();   
				while (iterator_.hasNext()){   
					Entry<Tac, Temp> entry = iterator_.next();
					if(!bb.liveInExt.containsKey(entry.getKey())){
						bb.liveInExt.put(entry.getKey(), entry.getValue());
						changed = true;
					}
				}
