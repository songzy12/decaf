...
import decaf.error.BadCaseError;
...
import decaf.error.NotLValueError;
...
public class TypeCheck extends Tree.Visitor {
	...
	public void visitCondExpr(Tree.CondExpr condExpr) {
		checkTestExpr(condExpr.cond);
		condExpr.left.accept(this);
		condExpr.right.accept(this);
		if (condExpr.left.type.equal(BaseType.ERROR) || condExpr.right.type.equal(BaseType.ERROR))
			condExpr.type = BaseType.ERROR;
		else if (!condExpr.left.type.equal(BaseType.ERROR) && !condExpr.right.type.equal(BaseType.ERROR)
				&& !condExpr.left.type.equal(condExpr.right.type)) {
			issueError(new CondExprTypeError(condExpr.cond.getLocation(), condExpr.left.type.toString(), condExpr.right.type.toString()));
			condExpr.type = BaseType.ERROR;
		}
		else
	...
	public void visitPreinc(Tree.Preinc preinc) {
		preinc.expr.accept(this);
		if (preinc.expr.type.equal(BaseType.ERROR))
			preinc.type = BaseType.ERROR;
		else if (!preinc.expr.type.equal(BaseType.INT)) {
			preinc.type = BaseType.ERROR;
			issueError(new IncompatUnOpError(preinc.getLocation(), "++", preinc.expr.type.toString()));
		}
		else if (preinc.expr instanceof Tree.Indexed || preinc.expr instanceof Tree.Ident ||
				preinc.expr instanceof Tree.Preinc || preinc.expr instanceof Tree.Predec)
			preinc.type = BaseType.INT;
		else {
			preinc.type = BaseType.ERROR;
			issueError(new NotLValueError(preinc.getLocation(), "++"));
		}
	}

	@Override
	public void visitPredec(Tree.Predec predec) {
		predec.expr.accept(this);
		if (predec.expr.type.equal(BaseType.ERROR))
			predec.type = BaseType.ERROR;
		else if (!predec.expr.type.equal(BaseType.INT)) {
			predec.type = BaseType.ERROR;
			issueError(new IncompatUnOpError(predec.getLocation(), "--", predec.expr.type.toString()));
		}
		else if (predec.expr instanceof Tree.Indexed || predec.expr instanceof Tree.Ident ||
				predec.expr instanceof Tree.Preinc || predec.expr instanceof Tree.Predec)
			predec.type = BaseType.INT;
		else {
			predec.type = BaseType.ERROR;
			issueError(new NotLValueError(predec.getLocation(), "--"));
		}
	}

	@Override
	public void visitPostinc(Tree.Postinc postinc) {
		postinc.expr.accept(this);
		if (postinc.expr.type.equal(BaseType.ERROR))
			postinc.type = BaseType.ERROR;
		else if (!postinc.expr.type.equal(BaseType.INT)) {
			postinc.type = BaseType.ERROR;
			issueError(new IncompatUnOpError(postinc.getLocation(), "++", postinc.expr.type.toString()));
		}
		else if (postinc.expr instanceof Tree.Indexed || postinc.expr instanceof Tree.Ident ||
				postinc.expr instanceof Tree.Preinc || postinc.expr instanceof Tree.Predec)
			postinc.type = BaseType.INT;
		else {
			postinc.type = BaseType.ERROR;
			issueError(new NotLValueError(postinc.getLocation(), "++"));
		}
	}

	@Override
	public void visitPostdec(Tree.Postdec postdec) {
		postdec.expr.accept(this);
		if (postdec.expr.type.equal(BaseType.ERROR))
			postdec.type = BaseType.ERROR;
		else if (!postdec.expr.type.equal(BaseType.INT)) {
			postdec.type = BaseType.ERROR;
			issueError(new IncompatUnOpError(postdec.getLocation(), "--", postdec.expr.type.toString()));
		}
		else if (postdec.expr instanceof Tree.Indexed || postdec.expr instanceof Tree.Ident ||
				postdec.expr instanceof Tree.Preinc || postdec.expr instanceof Tree.Predec)
			postdec.type = BaseType.INT;
		else {
			postdec.type = BaseType.ERROR;
			issueError(new NotLValueError(postdec.getLocation(), "--"));
		}
	}
	...
	public void visitRepeat(Tree.Repeat repeat) {
		...
		breaks.pop();
		checkTestExpr(repeat.expr);
	}
	
	@Override
	public void visitSwitch(Tree.Switch switch_) {
		switch_.expr.accept(this);
		if (!switch_.expr.type.equal(BaseType.ERROR) && !switch_.expr.type.equal(BaseType.INT))
			issueError(new BadSwitchError(switch_.expr.getLocation(),
					switch_.expr.type.toString()));
		breaks.add(switch_);
		for (Tree s : switch_.caseList)
			s.accept(this);
		if (switch_.defaultClause != null)
	...
	public void visitCase(Tree.Case case_) {
		...
		if (!case_.expr.type.equal(BaseType.ERROR) &&
			!(case_.expr.type.equal(BaseType.INT) && case_.expr instanceof Tree.Literal))
			issueError(new BadCaseError(case_.expr.getLocation()));
		for (Tree s : case_.slist)
			s.accept(this);
	}

	@Override
	public void visitDefaultClause(Tree.DefaultClause default_) {
		for (Tree s : default_.slist)
			s.accept(this);
	}
